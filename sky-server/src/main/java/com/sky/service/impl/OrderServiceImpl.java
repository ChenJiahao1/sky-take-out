...
//                    new BigDecimal(0.01));//原订单金额
    ...
    public void repetition(Long id) {
        ...
        for (OrderDetail x : orderDetailList) {
            ...
            //！！新增功能
            //判断菜品/套餐是否停售
            Long dishId = x.getDishId();
            Long setmealId = x.getSetmealId();

            if (dishId != null) {
                Dish dish = dishMapper.getById(dishId);
                if (dish.getStatus() == StatusConstant.DISABLE) {
                    log.info("再来一单业务中订单id:{}中存在菜品:{}停售", id, dishId);
                    continue;
                }
            } else {
                Setmeal setmeal = setmealMapper.getById(setmealId);
                if (setmeal.getStatus() == StatusConstant.DISABLE) {
                    log.info("再来一单业务中订单id:{}中存在套餐:{}停售", id, setmealId);
                    continue;
                }
            }
...
//        }).collect(Collectors.toList());
        ...
        // 将购物车对象批量添加到数据库
        if (shoppingCartList.size() > 0) shoppingCartMapper.insertBatch(shoppingCartList);
    }
    ...
    public OrderStatisticsVO statistics() {
        ...
        log.info("管理端各个状态订单统计, 待接单:{}, 待派送:{}, 派送中:{}", toBeConfirmed, confirmed, deliveryInProgress);
...
//            log.info("申请退款：{}", refund);
            ...
            log.info("申请退款:{}", orderDB.getAmount());
        }
